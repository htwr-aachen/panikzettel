...
Als \textbf{Konfiguration} einer TM ist ein String $\alpha q\beta$ mit $q \in Q, \alpha,\beta \in \Gamma^* $ und bedeutet, dass die TM sich im Zustand $q$ befindet, während der Lesekopf auf dem ersten Symbol von $\beta$ und eine Position hinter dem letzten Symbol $\beta$ ruht. $\alpha_1\ldots \alpha_n \underbrace{\beta_1}_{\text{Kopf}}\ldots \beta_n$. 
...
Hierbei muss der Verifizierer $V$ in polynomieller Zeit das Zertifikat $y$ prüfen können. Das Zertifikat ist durch die Eingabe polynomiell ($p$) in der Länge begrenzt.
...
\hline
Halteproblem $H$ & $H=\{\langle M\rangle w \mid \text{$M$ hält auf $w$}\}$ & semi \\
\hline
$\comp H$ & $\comp H = \{\langle M \rangle w \mid \text{$M$ hält nicht auf $w$}\}$ & \xmark \\
\hline
spez. Halteprob. $H_\varepsilon$ & $H_\varepsilon = \{\langle M \rangle \mid \text{$M$ hält auf $\varepsilon$}\}$ & semi \\
\hline
$\comp H_\varepsilon$ & $\comp H_\varepsilon = \{\langle M\rangle \mid \text{$M$ hält nicht auf $\varepsilon$}\}$ & \xmark \\
\hline
$H_\text{all}$ & $H_\text{all} = \{\langle M \rangle \mid \text{$M$ hält auf jeder Eingabe}\}$ & \xmark \\
\hline
$\comp H_\text{All}$ & $\comp H_\text{All} = \{\langle M \rangle \mid \text{$M$ hält auf keiner Eingabe}\}$ & \xmark \\ 
\hline
...
\hline
\NPC & \textsc{Sat} & Lösbarkeit logischer Formeln  \\
\hline
\NPC & \textsc{Clique} & Hat ein Graph eine Clique?   \\
\hline
\NPC & \textsc{TSP-E} & Gibt es eine Eulertour einer maximalen Länge?  \\
\hline
\NPC & \textsc{Knapsack-E} & Gibt es Objekte unter einer Gewichtsschranke mit min. Nutzen? \\
\hline
