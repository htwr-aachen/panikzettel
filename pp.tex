...
\begin{defi}{$\omega$-invariants}
	...
	\[
		I_0 \leq \Phi_f(\mathbf{0}) \text{ and } I_{n+1} \leq \Phi_f(I_n) \qquad\text{ for all } n \in \nat.
	\]
...
\begin{halfboxr}
	...
	\begin{theo}{Properties of $\cwp$}
		...
		\bigskip
		\begin{itemize}[leftmargin=*]
			\item {\large{}\textsc{Decoupling}}: \\[0.4em]
...
\begin{defi}{Hoisting}
	\begin{align*}
		...
		T(\stmtDiverge,f)           & = (\stmtDiverge,\mathbf{1})                                                                                        \\
...
\begin{theo}{Correctness of hoisting}
	...
	$$\cwp[\![P]\!](f,\mathbf{1}) = \wp[\![Q]\!](f) \text{  with  } T(P,\mathbf{1})=(Q,h).$$
...
\begin{halfboxr}
	...
	\begin{defi}{Decision problems $AST$ and $UAST$}
		...
		\begin{align*}
			(P,s) \in AST & \text{ iff } \wp[\![P]\!](\mathbf{1})(s)=\mathbf{1}  \\
...
\begin{theo}{Proof rule for almost-sure termination (AST)}
	...
	$$[I] \leq \wp[\![\stmtWhile{G}{P}]\!](\mathbf{1})$$
\end{theo}
	...
	\begin{theo}{Properties of $\ert$}
		\begin{itemize}[leftmargin=*]
			...
			\item \textsc{Connection to wp}: \\ \hspace*{1em}
			      $\ert[\![P]\!](t)=\ert[\![P]\!](\mathbf{0})+\wp[\![P]\!](t)$.
			\item \textsc{Affinity}: \\
			      {\scriptsize{}$\ert[\![P]\!](a {\cdot} t {+} t') {=} \ert[\![P]\!](\mathbf{0})+ a \cdot \wp[\![P]\!](t) + \wp[\![P]\!](t') $.}
		\end{itemize}
	\end{theo}
\end{halfboxr}
...
\begin{center}
	...
	\begin{tabular}{L|C}
		...
		\stmtSkip              & \mathbf{1}+t                                                                \\
		\stmtDiverge           & \infty                                                                      \\
		\stmtAsgn{x}{E}        & \mathbf{1}+t[x:=E]                                                          \\
		\stmtRasgn{x}{\mu}     & \mathbf{1}+\lam{s} \int_\rat (\lam{v}t(s[x := v]))~\mathrm{d}\mu_s          \\
		\stmtSeq{P_1}{P_2}     & \ert[\![P_1]\!](\ert[\![P_2]\!](t))                                         \\
		\stmtIf{G}{P_1}{P_2}   & \mathbf{1}+ [G] \cdot \ert[\![P_1]\!](t + [\neg G] \cdot \ert[\![P_2]\!](t) \\
		\stmtProb{p}{P_1}{P_2} & \mathbf{1}+ p \cdot \ert[\![P_1]\!](t) + (1-p) \cdot \ert[\![P_2]\!](t)     \\
		\stmtWhile{G}{P}       & \lfp X. (\mathbf{1}+ [G] \cdot \ert[\![P]\!](X) +[\neg G] \cdot t)
	\end{tabular}
\end{center}
...
\begin{defi}{Conditional independence}
	Let $X,Y,Z$ be (discrete) random variables. $X$ is \emph{conditionally independent} of $Y$ given $Z$, denoted $X \perp\!\!\!\!\perp Y \mid Z$, whenever:
...
\begin{theo}{Graphoid axioms}
	...
	\begin{enumerate}
		\item Symmetry: $X \perp\!\!\!\!\perp Y \mid Z$ iff $Y \perp\!\!\!\!\perp X \mid Z$
		\item Decomposition: $(X \perp\!\!\!\!\perp Y \cup W) \mid Z$ implies ($X \perp\!\!\!\!\perp Y \mid Z$ and $X \perp\!\!\!\!\perp W \mid Z$)
		\item Weak union: $(X \perp\!\!\!\!\perp Y \cup  W) \mid Z$ implies $X \perp\!\!\!\!\perp W \mid (Z \cup Y)$
		\item Contraction: ($X \perp\!\!\!\!\perp Y \mid Z$ and  $X \perp\!\!\!\!\perp W \mid (Z \cup Y)$) implies $(X \perp\!\!\!\!\perp Y \cup W) \mid Z$
		\item Triviality: $X \perp\!\!\!\!\perp \emptyset \mid Z$
	\end{enumerate}
\end{theo}
...
\begin{halfboxl}
	...
	And since $dsep_G(X,Y,Z)$ implies $X \perp\!\!\!\!\perp Y \mid Z$, we can sometimes provide a guarantee that sets of nodes are conditionally independent in polynomial time.
\end{halfboxl}%
\begin{halfboxr}
	\vspace{-\baselineskip}
	\begin{defi}{d-seperation}
		Let $X,Y,Z$ be disjoint sets of vertices in the DAG $G$.
		$X$ and $Y$ are \emph{d-seperated} by $Z$ in $G$, denoted $dsep_G(X,Y,Z)$, iff:
...
$$\Pr(v \mid w \land \partial v) = \Pr(v \mid \partial v) \text{ which is equivalent to } v \perp\!\!\!\!\perp w \mid \partial v.$$
