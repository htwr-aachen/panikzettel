...
\skript{1.51} (Steinitzscher Austauschsatz) Für ein linear unabhängiges Tupel $s$ in $V$ und ein Erzeugendensystem $t$ mit Länge $n$ von $V$ ist das folgende Tupel ein Erzeugendensystem von $V$:
$$(t_1, \ldots, s_1, t_i, \ldots, t_j, s_k, \ldots, t_n)$$
		...
		child { node {$\det(A) = \det(B)$}
			child { node {$\chi_A = \chi_B$}
				child { node[align=center] {$\chi_A$ zerfällt in Linearfaktoren \\ $\mul_a(A) = \geo_a(A)$ \\ $\mul_a(B) = \geo_a(B)$ \\ $\forall$ EW $a$ von $A$ bzw. $B$}
					child { node {ähnlich} }
					child { node {$P \cdot A = B \cdot P$ lösbar}
						child { node { ähnlich } }
