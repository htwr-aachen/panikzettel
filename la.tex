...
\skript{1.78} Für einen Untervektorraum $U$ von $V$ heißt $U^\prime$ Komplement von $U$ wenn $$V = U \dot+ U^\prime$$
...
\skript{2016 HA. 24 c)} $\varphi(\langle s_1,\ldots,s_n \rangle) = \langle \varphi(s_1), \ldots, \varphi(s_n) \rangle$

\skript{2016 HA. 24 d),e)} Wenn $(\varphi(s_1),\ldots,\varphi(s_n))$ linear unabhängig ist, dann ist auch $(s_1,\ldots,s_n)$ linear Unabhängig. Ist $\varphi$ injektiv, so gilt auch die Umkehrung.
...
\skript{3.50} Für $\varphi : V \to W$ sollen Basen $(s_1, \ldots, s_n)$ von $V$ und $(t_1, \ldots, t_m)$ von $W$ bestimmt werden, sodass $M_{t,s}(\varphi) = Q_r$ mit $r \leq \min(n, m)$. \begin{enumerate}
...
\skript{6.56} Für $f \in K[X]$ und $A \in K^{n \times n}$ mit einem $P \in \GL_n(K)$ sodass
...
\skript{6.77} Für einen $K$-Vektorraumendomorphismus $\varphi: V \to V$ heißt ein Untervektorraum $U$ von $V$ invariant unter $\varphi$ wenn
$$\varphi(U) \subseteq U$$
