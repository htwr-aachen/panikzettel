...
This characteristic time for the SIS-model is given with
\begin{align*}
...
The \textit{intrinsic value} measures the good's value on its own with respect to the customer.
The \textit{network value} measures the value a customer gets by other users using the product.
...
% TODO Explain equilibrium price
...
If every node in the network adopts $ A $ the chain reaction is called \textit{complete cascade}.
