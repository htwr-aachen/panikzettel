...
\begin{figure}[ht!]
	\centering
	\begin{tikzpicture}
		\tikzstyle{state} = [draw, ultra thick, rounded rectangle, minimum height=2cm,minimum width=4cm];
		
		\node[state,draw=bluishgreen,fill=bluishgreen!20] (s) at (0,0) {Susceptible};
		\node[state,draw=reddishpurple,fill=reddishpurple!20] (i) at (6,0) {Infected};
		
		\path[->,ultra thick] (s) edge (i);
	\end{tikzpicture}
\end{figure}
