...
The model adds one of the two states \textit{susceptible} and \textit{infected} to each node.

Consider a disease that spreads in population of $ N $ individuals.
We define
\begin{itemize}
	\item $ S(t) $: the number of individuals susceptible at time $ t $
	\item $ I(t) $: the number of individuals infected at time $ t $
	\item $ s(t) = \frac{S(t)}{N} $: fraction of susceptible population at time $ t $
	\item $ i(t) = \frac{I(t)}{N} $: fraction of infected population at time $ t $
	\item $ \beta $: likelihood that an individual infects a susceptible neighbor
\end{itemize}

% TODO write some more detail hered
