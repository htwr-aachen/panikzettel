...
Wer soll das Lernen?
Verzweifle nicht.
Wir haben hier nur etwas anders gemacht als sonst: Wir haben fast alle Stichworte aus der Vorlesung mal erwähnt, von denen man wahrscheinlich in der Klausur was gehört haben sollten könnte.
...
Deinen Fokus solltest du auf die Abschnitte Diagrammtypen (\ref{sec:diagrammtypen}) und Vorgehensmodelle (\ref{sec:vorgehensmodelle}).
...
Die drei \emph{Akteure}, Autoren, Korrektoren\footnote{Weil wir keine Fehler machen, momentan nicht vorhanden.} und Studis interagieren mit dem System und haben \emph{nehmen an} Use-Cases \emph{teil}.
...
\textbf{Scrum:} ``Agile Methode''. Inkrementell und iterativ entwickeln und durch Transparenz, Überprüfung und Anpassung während jedem Zeitpunkt der Entwicklung.
