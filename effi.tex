...
Wir betrachten im Folgenden randomisierte Algorithmen zur Bestimmung minimaler Schnitte $C^*$, das heißt
\[\Forall U \subset V : c(U) \ge c(C^*).\]

Da wir einen minimalen Schnitt suchen, gehen wir davon aus, dass dieser klein sein wird.
Wir wählen also zufällig Kanten aus und \emph{kontrahieren} sie, d.h.\ verschmelzen die anliegenden Knoten.
...
\begin{theo}{Laufzeit von Schöning}
    Der Algorithmus von Schöning hat eine Laufzeit von $\bigo(m \cdot n^{3/2} \cdot \big(\frac{4}{3}\big)^n)$. Es können nur false negatives auftreten.
\end{theo}
