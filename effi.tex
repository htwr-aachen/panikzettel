...
Die Anzahl der Iterationen ist maximal $n-1$: In jedem Schritt werden alle Wege der Länge $l = \dist_{G_f'}(s,t)$ getrennt. Neu entstehende Kanten führten vorher von Schicht $i$ zu $i-1$, damit müssen neue Wege mindestens Länge $l+2$ haben.
\end{halfboxl}%
\begin{halfboxr}
\vspace{-\baselineskip}
\begin{algo}{Dinitz}
...
\begin{halfboxl}
Ein Sperrfluss kann in $\bigo(n^2)$ mit \emph{Forward-Backward-Propagation}  bestimmt werden.
...
Jede Propagationsphase läuft in $\bigo(n+l)$ mit $l$ Anzahl neu saturierter Kanten. Nach maximal $n-1$ Propagationsphasen ist ein Sperrfluss gefunden. Also Laufzeit für Forward-Backward-Propagation: $\bigo(\sum_{i=1}^{n-1} (n+l)) = \bigo(m + n^2)$.
\end{halfboxl}%
\begin{halfboxr}
\vspace{-\baselineskip}
\begin{algo}{Forward-Backward-Propagation}
...
\begin{enumerate}
    ...
    \item Verbessere die Kosten:
        \begin{itemize}
            \item Solange es in $G_f$ einen Kreis $C$ mit negativen Kosten gibt:
                \begin{enumerate}
                    \item Setzte $C = \hyperref[algo:min-mean-cycle]{min-mean-cycle} (V)$
...
Wir werden Werte $d_k(v)$ für die Kosten des günstigsten Kantenzuges eines beliebigen Knoten nach $v$ mit Länge $k$ verwenden, wobei $v \in V, k \in \nat$.
...
\begin{enumerate}
    \item Bestimme $d_j(v)$ für $v \in V, 0 \le j \le n$:
        \begin{itemize}
            \item Setzte $d_0(v) = 0$ für alle $v \in V$.
...
Mit nicht maximalen Matching $M$ und Maximum Matching $M'$ gibt es in $M$ einen verbessernden Pfad der Länge maximal $2 \cdot \floor*{\frac{\abs{M}}{\abs{M'} - \abs{M}}} + 1$.
...
\begin{enumerate}
    ...
    \item Solange es verbessernde Pfade gibt:
        \begin{enumerate}
            \item Finde inklusions-maximale Menge von knotendisjunkten verbessernden Pfaden $P_1, \ldots, P_l$ der kürzesten Länge $l$.
            \item $M = M \oplus E(P_1) \oplus \ldots \oplus E(P_l)$.
        \end{enumerate}
\end{enumerate}
\end{algo}
\end{halfboxr}
...
Der Beweis geht über das Graphenprodukt $G^k = G \times C_k$ von zyklischen Graphen, wobei $C_k$ der Cliquengraph in der Iterartion $k$ ist.
...
\begin{enumerate}
    ...
    \item Wähle Vertex Cover $\bigcup_{e \in M} e$.
\end{enumerate}
\end{algo}
\end{halfboxr}
...
Beim \emph{Travelling Salesman Problem} soll in einem vollständigen Graph ein Hamiltonkreis gefunden werden, also einer, der über alle Knoten führt.
...
Das ursprünglihce Zentrumsproblem \emph{ohne Knotenkosten} kann erreicht werden, indem man die Knotenkosten auf $1$ setzt.
...
\tcblower
\begin{enumerate}
    \item Setzte $Z = \emptyset$ und $U = V$
    \item Solange $U \neq \emptyset$:
    \begin{enumerate}
...
\begin{enumerate}
    ...
    \item Bestimme mit \hyperref[subsec:binPacking]{Bin Packing Algorithmus} Zuteilung für $J'$, oder gebe \texttt{Makespan zu klein} zurück, wenn dies nicht möglich ist
...
Wie wir diese Jobs verteilen, werden wir in Teil~\ref{subsec:binPacking} betrachten. Aus dem dort beschriebenen Algorithmus erhalten wir entweder eine Verteilung auf die $m$ Maschienen mit Makespan $(1+\varepsilon)Z^\ast$ oder erkennen, dass es keine solche Zuweisung gibt.
...
In einem \emph{Linearen Programm (LP)} soll eine linare \emph{Zielfunktion} unter Einhaltung von linaren \emph{Nebenbedingungen} maximiert werden.
...
\begin{halfboxr}
\vspace{-\baselineskip}
\begin{theo}{Laufzeit mit Pertrubierung}
...
Die \emph{Ellipsoid-Methode} ist ein Algorithmus, der in polynomieller Laufzeit bestimmen kann, ob ein linares Ungleichungssystem eine Lösung hat, d.h. der Lösungsraum nicht leer ist.

Der Begriff der \emph{Kugel} und \emph{Ellipsoid} sind wichtig:
...
Ein lineares Ungleichungssystem $G$ mit Eingabenlänge $L$ kann in polynomieller Laufzeit in ein linares Ungleichungssystem $G'$ mit folgenden Eingenschaften umgewandelt werden:
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item Die Ellipsoid-Methode kann in polynomieller Zeit bestimmen, ob ein linares Ungleichungssystem eine Lösung hat.
\end{enumerate}
\end{theo}
\end{halfboxr}
...
\begin{enumerate}
    ...
    \item Setzte $\tau = (\alpha \cdot m)^{-2(m+1)}$.
    ...
    \begin{itemize}
        ...
        \item so setzte $S = S \cup \{ i \}$, $m' = m' +0$, und $\overline{\delta}(m') = i$.
    \end{itemize}
...
\vspace{-\baselineskip}
    \begin{defi}{c-competitive}
        Sei $C_A(\rho)$ die Kosten der von $A$ berechneten Strategie auf der Eingabe $\rho$ und $C^*(p)$ die Kosten für die optimale Strategie. Dann ist $A$ $\textit{c-competitive}$ gdw. $\exists a$ für jede Eingabe $\rho$, so dass gilt: $$C_A(\rho) \leq c \cdot C^*(\rho)+a.$$
    \end{defi}
\end{halfboxr}
...
Wir zeigen nun, dass kein Algorithmus competetive-Faktor kleiner $3$ hat.
...
Also sind die Gesamtkosten von $A$ die Summe der Kosten dieser Algorithmen. Da der optimale Algorithmus nicht schlechter sein kann, kann $A$ keinen competetive-Faktor kleiner $3$ haben.
...
\vspace{-\baselineskip}
    \begin{theo}{LIFO}
        \textit{LIFO} ist nicht $c$-competitive für beliebiges $c \in \mathbb{N}$.
    \end{theo}
\end{halfboxr}
...
\vspace{-\baselineskip}
    \begin{theo}{LFU}
        $LFU$ ist nicht $c$-competitive für beliebiges $c \in \mathbb{N}$.
    \end{theo}
\end{halfboxl}%
\begin{halfboxr}
    \vspace{-\baselineskip}
...
\subsubsection{Untere Schranke}
Wir beweisen nun das deterministische Verfahren nur begrenzt gut sein können.
...
Fülle nun beide Caches mit den geichen $k$ Seiten. Jetzt wird die Eingabesequenz so gewählt, dass $A$ immer die Seite nachlädt, die nicht im Cache ist, sondern als einzige nur im Hauptspeicher liegt. $B$ dagegen lagert diejenige Seite aus, die mindestens in den nächsten $k-1$ Schritten nicht angefragt wird.
...
\begin{defi}{c-competitive für Random}
    Ein randomisierter Online-Algorithmus $A$ ist c-competitive gdw. $\exists a$ , so dass für jede durch einen oblivous Gegner bestimmte Anfragesequenz $\rho$ gilt: $$\mathbb{E}[C_A(\rho)] \leq c \cdot C^*(\rho)+a.$$
\end{defi}
...
\begin{defi}{$(m,c)$-competitive}
    Ein Online-Algorithmus $A$ ist $(m,c)-competitive$ gdw.
    \begin{itemize}
        \item $m$-mal so großer Cache wie Offline,
        \item für jede Eingabesequenz $\rho$ $\exists a$, sodass $C_A(\rho) \leq c \cdot C^*(\rho)+a$.
    \end{itemize}
\end{defi}
\end{halfboxr}
...
Wir definieren $\phi$ als die Anzahl der Seiten, die im Cache von $OPT$ sind, aber nicht im Cache von $Random$. Dann kann $\phi$ nicht negativ sein, also reicht die obige Invariante, um den competetive-Faktor zu zeigen.
...
\begin{itemize}
    ...
    Man könnte den competetive-Faktor noch besser begründen.
