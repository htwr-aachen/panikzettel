...
\end{tightcenter}
Together with the Bayes Rule, we obtain the \emph{Bayesian update} as follows:
\begin{tightcenter}
...
\begin{center}
\begin{tikzpicture}
\node[draw] (A) at (0,0) {$A(x)$};
\node[draw] (B) at (3,0) {$B(x) \land C(x)$};
\node[draw] (Y1) at (0,-1.5) {Yes};
\node[draw] (Y2) at (3,-1.5) {Yes};
\node[draw] (N) at (6,0) {No};
\path[->] (A) edge[above] node {N} (B);
\path[->] (A) edge[right] node {Y} (Y1);
\path[->] (B) edge[above] node {N} (N);
\path[->] (B) edge[right] node {Y} (Y2);
\end{tikzpicture}
\end{center}
