...
Wenn man ein ER-Diagramm in ein Relationales Modell überführt gilt es verschiedene Fälle zu beachten. Um die Form der Relation zu verdeutlichen werden Beispiele aus der Luft gegriffen. Grundsätzlich gilt:
\begin{itemize}
...
\begin{itemize}
\item \textbf{1:n Beziehung}: \textbf{keine} eigene Relation für die Beziehung anlegen, sondern Primärschlüssel des Objekts, das nur einmal auftritt, als \emph{Attribut} in die Relation des $n$-mal auftretenden Objekts aufnehmen. Also z.B. $n$-Studenten, die auf 1 Webseite zugreifen:
\begin{itemize}
...
\end{itemize}
\item \textbf{Rekursive Beziehung}: Jede \emph{Version} im Repository hat Vorgänger und Nachfolger, die selbst wieder eine Version sind:
\begin{itemize}
...
\item \textbf{1:1 Beziehung}: Gegebenenfalls zu einem Objekt verschmelzen, nur einen der Primärschlüssel als Primärschlüssel behalten.
\end{itemize}
...
\end{itemize}
\item \textbf{Schwache Entity Typen}: Fremdschlüssel des starken Objekts als Attribut in der Relation des Schwachen und Interrelation zu diesem (wie beim mehrwertigen Attribut).
\end{itemize}
\begin{itemize}
\item \textbf{Generalisierung/Spezialisierung}:
\begin{itemize}
\item Relationen für die Objekte formulieren.
\item Wenn Generalisierung disjunkt: Student $\cap$ EhrlichUndHartArbeitenderMensch = $\varnothing$.
...
=======
\item Finde nichttriviale Abhängigkeit $\alpha \to \beta \in F_i^+$ mit $\alpha \cap \beta = \emptyset$.
>>>>>>> Vorschlag zu 3.1 ER Diagramme überführen. Kleine Tippfehler in 5.4.4 und 6.2.1 korrigiert.
