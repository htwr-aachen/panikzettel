...
\author{Philipp Schröer, Younes Müller, Jonas Schneider}
...
This Panikzettel is about the lecture Operations Research 1 by Prof.\ Lübbecke held in the winter semester 2025/26.

This Panikzettel is Open Source. We appreciate comments and suggestions at \\ \url{https://github.com/htwr-aachen/panikzettel}.
\bigskip

If you are indeed in panic, please only read this Panikzettel once or twice and \textbf{do not} learn it by heart, instead try to model various problems and exercise, since that are main points in the exam.

In this Panikzettel, we introduce new notation for better readability: $\overline{n} := \set{1,\ldots, n}$ for some $n \in \mathbb{Z}$. Please not that this is only our notation and should not be used in the exam, especially not without prior definition.
...
\begin{halfboxr}
	...
	\begin{defi}{Mixed-Integer Linear Program}
		A \emph{mixed-integer linear program (MIP)} has the following form, where $n,~ q \in \nat$:
		...
		Where $\mathbb{D}$ is either $\mathbb{Q}$ for a \emph{linear program} or $\mathbb{Z}$ for an \emph{integer program}. It could also have both for a Mixed Integer Program.
	\end{defi}
\end{halfboxr}
...
\begin{halfboxl}
	...
	\begin{itemize}
		\item $X = Y \land Z$: $x \leq y$, $x \leq z$, $x \geq y + z - 1$
...
\begin{halfboxl}
	...
	Outgoing edges from $i$ are denoted $\delta^+(i)$, and incoming edges are denoted $\delta^-(i)$.
\end{halfboxl}%
\begin{halfboxr}
	\vspace{-\baselineskip}
	\begin{defi}{Flow}
...
Use one variable for each feasible \emph{pattern}: One pattern $x \in \set{0,1}^n$ for every combination of items that can be packed into a single bin (we disallow any patterns that would exceed the bins capacity):
\[
	Q = \Set{x \in \set{0,1}^n | \sum_{i=1}^{n} a_i  \cdot x_i \leq b} = \Set{\begin{pmatrix}1 \\ \vdots \\ 0\end{pmatrix}, \dots, \begin{pmatrix}0 \\ \vdots \\ 1\end{pmatrix}, \dots, \begin{pmatrix}1 \\  1  \\ \vdots\end{pmatrix}}
\]
...
\subsection{Lot Sizing (sketch) [not covered in WS25/26]}
...
Alternatively construct a directed graph $G' = (V, A = {(i,j),(j,i) \mid {i,j} \in E})$. Fix some $r \in V$ as the root of the tree.
\begin{linearprogram}
	\lpVars{x_{ij} \in \Set{0,1}}{\Forall ij \in E}{$x_{ij} = 1$ iff edge $i,j$ is in the MST}
	\lpMoreVars{f_{ij} \ge 0}{\Forall ij \in A}{$f_{ij}$ flow of the edge $i,j$}
	\lpMin{\sum_{ij \in E} c_{ij} \cdot x_{ij}}
	\lpWhere{\sum_{ij \in E} x_{ij} = \abs{V} - 1}{}{spanning tree}
	\lpConstraint{\sum_{ij \in \delta^+(i)} f_{ij} - \sum_{ji \in \delta^-(i)} f_{ij} = \begin{cases} \abs{V} - 1 & i = r \\ -1 & i \neq r\end{cases}}{\Forall i \in V}{flow conservation}
	\lpConstraint{f_{ij} + f_{ji} \le (\abs{V} - 1) \cdot x_{ij}}{\Forall ij \in E}{use edge when there is flow}
\end{linearprogram}

We assume a flow from then root as the source to each node (since we construct a tree over all nodes). Each node requires a virtual flow of 1, thus the root node produces $\abs{V} - 1$ flow, and the outgoing flow $f_{ij},\quad ij \in \delta^+(i)$ minus the ingoing flow $f_{ij},\quad ji \in \delta^-(i)$ should be $-1$.

\vspace{-0.5\baselineskip}

\subsection{Traveling Salesperson Problem (Dantzig, Fulkerson and Johnson)}
\label{sec:tsp}
...
\subsubsection*{Alternative Subtour Elimination Constraint (Miller, Tucker and Zemlin)}
...
\begin{halfboxl}
	...
	And a \emph{cutting plane} is a valid inequality that cuts away a (optimal) solution $x^\ast$ in the LP relaxation (one that is not integral).
	\textbf{Proof both cutting \& validity in the exam!}.
\end{halfboxl}%
\begin{halfboxr}
	\vspace{-\baselineskip}
...
\begin{bonus}{Example Branch-and-Bound Tree}
	An example for a branch-and-bound tree could be the following (the node names represent the search order not solution or value).
	Here 1,2,4 split the problem into the parts of the LP relaxation solution which were not integer (i.e. for 1, e.g. $x_1 = 3, x_2 = 1.5$).
	Then some branches are infeasible even for the LP relaxation like 3. Then, depending on the search order, we find 5 as the first integer solution. This is quite lucky since it will be later proven to be optimal.
	Then the local lower bound (i.e. the solution of the LP relaxation + extra inequalities of the tree until there) of 6 are already lower than the best currently known integer solution,  thus we can prune it by bound.
	This could be also true for 7.
	Then we have either pruned or evaluated all open branches and can determine optimality for our best integer solution: the solution of 5.
	...
	\begin{center}
		\begin{tikzpicture}[
				...
				nde/.style={circle, draw,dashed, minimum size=0.5cm, inner sep=0pt, font=\large},
				inde/.style={circle, draw, minimum size=0.5cm, inner sep=0pt, font=\large, thick},
				unde/.style={diamond, draw, minimum size=0.5cm, inner sep=0pt, font=\large}
			]
			...
			child {
					...
					child {
							...
							child{
									node[nde] {6}
									node[below=5pt] {pruned by bound}
									edge from parent
									node[left] {$x_2 \le 0$}
								}
							child{
									node[inde] {5}
									node[below=5pt] {optimal}
									edge from parent
									node[right] {$x_2 \ge 1$}
								}
							edge from parent
...
% TODO: ADD Example Branch and Bound with Tree

\subsection{Solving LPs With Exponentially Many Constraints (little exam relevance)}
...
\subsection{0-1 Knapsack Problem with Bellman's Equation / Dynamic Programming}
