...
\begin{itemize}
	...
	\item $\{\land,\lor,0,1\}$ ist nicht funktional vollständig, da die Negation nicht allgemein darstellbar ist.
\end{itemize}
...
Ein \emph{Satz} ist eine Formel, in der keine freien Variablen vorkommt.
...
Um zu zeigen, dass eine Klasse $\K$ hingegen nicht bzw. nicht endlich axiomatisierbar ist, bieten sich oft einige Standardverfahren an, die hier kurz erläutert werden sollen:

\subsubsection{Nicht axiomatisierbar}
\begin{itemize}
	\item Kompaktheitssatz: Finde zunächst ein unendliches Axiomensystem $\Psi$ für eine Strukturklasse $K^\prime$ mit $K\cap K^\prime = \emptyset$, so dass jede endliche Teilmenge von $\Psi$ ein Modell in $K$ besitzt. 
	...
	Die Schwierigkeit ist hierbei die Wahl von $\Psi$. Oftmals bietet es sich an, $\Psi$ als Axiomensystem für die Klasse von unendlichen Strukturen oder für die Klasse von Graphen mit unendlichen Pfaden o. ä. zu wählen.
	
	Beispiel: Für $\K$ die Klasse der Graphen mit beliebig langen endlichen Pfaden, aber ohne unendlichen Pfad wähle $K^\prime$ als Klasse der Graphen mit unendlichem Pfad und konstruiere $\Psi$ entsprechend.
	
	\item Ehrenfeucht-Fra\"issé ($\tau$ abzählbar und relational): Finde zwei Strukturen $\A \in \K,\ \B \notin \K$ mit $\A \equiv \B$ und gebe eine Gewinnstrategie für die Duplikatorin für $G(\A,\B)$ an. Dann ist $\K$ nicht axiomatisierbar, da $\A$ und $\B$ nicht in $\FO$ unterschieden werden können.
	...
	\item Aufsteigender Satz von Löwenheim-Skolem: Hier gibt es zwei Fälle.
	\begin{itemize}
		\item In $\K$ sind bel. große, aber nur endliche Strukturen. Wenn ein hypothetisches $\Phi$ $\K$ axiomatisiert, hat $\Phi$ nach LS$\uparrow$ ein unendliches Modell, dies widerspricht aber der Definition von $\Phi$.
	...
	\item Absteigender Satz von Löwenheim-Skolem: Wenn $\K$ nur überabzählbar große Strukturen enthält, ist $\K$ nach LS$\downarrow$ nicht axiomatisierbar. Hierfür muss $\tau$ abzählbar sein.
\end{itemize}

\subsubsection{Nicht endlich axiomatisierbar}

\begin{itemize}
	\item Kompaktheitssatz: Dazu benötigen wir ein unendliches Axiomensystem $\Phi$ für $\K$, wobei jede endliche Teilmenge von $\Phi$ kein Axiomensystem für $\K$ ist (bei entsprechenden Aufgaben soll oft zuerst die generelle Axiomatisierbarkeit überprüft werden, daher ist ein solches $\Phi$ oft schon vorhanden).
