...
\usepackage{fix-cm}            % Optional font fixes

% Reset math font definitions
...
\subsection{Endlichkeitssatz (Kompaktheitssatz) der Aussagenlogik}

Den Endlichkeitssatz haben wir mithilfe des Vollständigkeitssatz der Prädikatenlogik bewiesen:
Behauptung: Zu zeigen ist: Wenn $\Phi \models \varphi$ gilt, dann existiert eine endliche Teilmenge $\Phi_0 \subseteq \Phi$, für die ebenfalls $\Phi_0 \models \varphi $ gilt.
\begin{itemize}
\item Annahme $\Phi \models \varphi$
\item Mit Vollständigkeitssatz von semantischer Folgerung zu syntaktischer Ableitung: $\Phi \vdash \varphi$
\item Ein formaler Beweis (eine Ableitung) ist per Definition immer eine endliche Kette von logischen Schritten. Daher kann er auch nur eine endliche Anzahl von Annahmen (Prämissen) aus der Menge $\Phi$ verwenden. Es muss also eine endliche Teilmenge $\Phi_0 \subseteq \Phi$ geben, die für den Beweis ausreicht. Daher gilt: $\Phi_0 \vdash \varphi$.
\item Mit Vollständigkeitssatz von syntaktischer Ableitung zu semantischer Folgerung: $\Phi_0 \models \varphi$
\end{itemize}
...
\begin{prooftree}
    ...
    \UnaryInfC{$\Gamma, \Gamma' \vdash \varphi$}
\end{prooftree}
\end{minipage}
...
\begin{minipage}{0.45\textwidth}\centering
\begin{prooftree}
    \AxiomC{$\Gamma \vdash \varphi_1 \land \varphi_2$}
    \RightLabel{ für $i = 1, 2$}
...
\begin{minipage}{0.45\textwidth}\centering
\begin{prooftree}
    \AxiomC{$\Gamma \vdash \varphi_1 \lor \varphi_2$}
